...
function renderPanel(state: { pending: boolean; error?: string; errorCode?: string }): void {
  ...
  showResultPanel({
    ...
    anchorRect: lastRect,
    onClose: disposeSession,
    onFollowUp: handleFollowUp,
    onStop: stopGeneration,
    onRetry: retryLastRequest ?? undefined,
    onResnip: () => handleResnip(resnipSettings),
...
function handleFollowUp(text: string): void {
  ...
  startGeneration({
    ...
    kind: 'follow-up',
    userText: text,
    // If the first analysis never completed, re-send the image with the
    // question instead of issuing an imageless follow-up.
    transport: baseHistory.length === 0 ? 'analyze' : 'follow-up',
  });
}

function showCaptureFailure(messageCaptureId: string, message: string, code: string): void {
